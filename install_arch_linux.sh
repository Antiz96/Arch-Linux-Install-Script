...
echo "Installating Linux Kernel and base system. This might take a few minutes..." && pacstrap /mnt base linux linux-firmware && echo "Linux Kernel and Base system installed"
...
#Set Language
arch-chroot /mnt bash -c 'sed -i "s/#fr_FR.UTF-8 UTF-8/fr_FR.UTF-8 UTF-8/" /etc/locale.gen && locale-gen && echo "LANG=fr_FR.UTF-8" > /etc/locale.conf && echo "KEYMAP=fr" > /etc/vconsole.conf' && echo "Language OK"
...
arch-chroot /mnt bash -c 'pacman -S --noconfirm sudo && sed -i "s/# %wheel ALL=(ALL) ALL/%wheel ALL=(ALL) ALL/" /etc/sudoers' && echo "sudo installed and configured"

#Install and configure GRUB
echo "installing and configuring GRUB. This might take a few minutes..." && arch-chroot /mnt bash -c 'pacman -S --noconfirm grub efibootmgr dosfstools mtools && mkdir /boot/EFI && mount /dev/sda1 /boot/EFI && grub-install --target=x86_64-efi --bootloader-id=grub_uefi --recheck && grub-mkconfig -o /boot/grub/grub.cfg' && echo "GRUB installed and configured"

#Install and enable Network Manager
arch-chroot /mnt bash -c 'pacman -S --noconfirm networkmanager && systemctl enable NetworkManager' && echo "Network Manager installed and enabled"

#Install necessary/useful packages for the base of the system + Gnome desktop environment
echo "Installing additional packages, drivers and DE. This might take a few minutes..." && arch-chroot /mnt bash -c 'pacman -S --noconfirm base-devel linux-headers intel-ucode nvidia xorg gnome && systemctl enable gdm' && echo "Necessary/useful packages and Gnome Destop Environment installed"
...
arch-chroot /mnt bash -c 'grub-mkconfig -o /boot/grub/grub.cfg' && echo "Grub configuration updated"
