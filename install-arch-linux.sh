...
#Define additionnal packages to install (such as useful packages for the system, Desktop environment, display manager, etc... Don't forget to modify "the systemctl enable" part depending on your choices"
PACKAGES() {
	pacman -S --noconfirm --needed networkmanager vim base-devel linux-headers xorg gnome > /dev/null 2>&1 && systemctl enable NetworkManager > /dev/null 2>&1 && systemctl enable gdm > /dev/null 2>&1
}
...
export CPU=$CPU && export GPU=$GPU && echo "Installing necessary and additional packages, drivers and Desktop environment. This might take a few minutes..." && arch-chroot /mnt bash -c 'pacman -S --noconfirm $CPU $GPU > /dev/null && PACKAGES' && echo "Packages installed" || exit 1
