...
#Create User
export USER_PWD=/"$USER_PWD/" && arch-chroot /mnt bash -c 'useradd -m $USERNAME && echo "$USERNAME:$USER_PWD" | chpasswd && usermod -aG wheel,audio,video,optical,storage,games $USERNAME' && echo "User created and configured" || exit 1
