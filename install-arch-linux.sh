...
# Set localtime
arch-chroot /mnt bash -c "ln -sf /usr/share/zoneinfo/$TIMEZONE /etc/localtime && hwclock --systohc" && echo "Timezone \"$TIMEZONE\" configured" || exit 1

# Set Language
arch-chroot /mnt bash -c "sed -i "s/#$LANGUAGE UTF-8/$LANGUAGE UTF-8/" /etc/locale.gen && locale-gen > /dev/null && echo "LANG=$LANGUAGE" > /etc/locale.conf && echo "KEYMAP=$KEYMAP" > /etc/vconsole.conf" && echo "Language \"$LANGUAGE\" configured" || exit 1
...
# Create User
arch-chroot /mnt bash -c "useradd -m $USER_NAME && echo "$USER_NAME:$USER_PWD" | chpasswd && usermod -aG wheel,audio,video,optical,storage,games $USER_NAME" && echo "User \"$USER_NAME\" created and configured" || exit 1
